...
const RatingSection = ({ booking, onRated }) => {
    ...
    return (
      <div className="mt-6 bg-white/10 border border-white/20 rounded-2xl p-6 backdrop-blur-sm relative z-10 shadow-inner">
        ...
        {booking.rating_comment && (
          <p className="text-sm text-emerald-50 mt-4 italic font-medium">
            "{booking.rating_comment}"
          </p>
        )}
        <p className="text-[10px] text-emerald-400 mt-4 font-black uppercase tracking-widest">
  ...
  return (
    <div className="mt-6 bg-white/5 border border-white/10 rounded-[2rem] p-6 relative z-10">
      <p className="text-sm font-black text-white mb-1 uppercase tracking-tighter">
        ⭐ Rate Your Operator
      </p>
...
const MyBookings = ({ userId }) => {
  ...
  const fetchMyBookings = async () => {
    ...
    try {
      ...
      setErrorStatus("Something went wrong while loading bookings.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatusDetails = (status) => {
    switch (status) {
      ...
      case "pending":
        return {
          color: "bg-white text-black border-indigo-500/30",
          icon: <Clock size={10} />,
  ...
  return (
    <div className="pt-36 pb-20 min-h-screen relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto px-6 relative z-10">
        ...
        {errorStatus && (
          <div className="mb-6 bg-red-500/20 border border-red-400/30 text-white rounded-2xl px-5 py-4">
            {errorStatus}
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="grid gap-6">
            {bookings.map((booking) => {
              ...
              return (
                <div
                  key={booking.id}
                  className="bg-gradient-to-br from-emerald-600/90 via-green-700/85 to-teal-900/90 p-8 rounded-[2.5rem] shadow-2xl border border-white/20 backdrop-blur-md text-white relative overflow-hidden group"
                >
                  ...
                  <div className="flex flex-col md:flex-row justify-between gap-6">
                    <div className="space-y-4 flex-1">
                      <div className="flex items-center gap-3 relative z-10">
                        <span
                          className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${status.color} shadow-sm border border-black/5`}
                        >
                      ...
                      <div className="grid grid-cols-2 gap-6 pt-2 relative z-10">
                        <div className="flex items-center gap-3 text-emerald-50 text-xs font-medium bg-white/5 p-3 rounded-xl border border-white/5">
                          ...
                          <span className="truncate">
                            {booking.address_line || booking.district || "—"}
                          </span>
                        </div>
                      ...
                      {booking.status === "completed" && booking.provider_id && (
                        <RatingSection booking={booking} onRated={fetchMyBookings} />
                      )}
                    </div>
          ...
          <div className="bg-gradient-to-br from-emerald-600/90 to-teal-900/90 rounded-[3rem] p-20 text-center border border-white/20 backdrop-blur-md shadow-2xl text-white relative overflow-hidden">
            ...
            <p className="text-emerald-100/70 font-medium max-w-md mx-auto leading-relaxed">
              You haven't placed any drone service requests. Your upcoming and past
      ...
      {selectedProvider && (
        <div className="fixed inset-0 bg-emerald-950/60 backdrop-blur-md flex items-center justify-center z-50 p-6">
          <div className="bg-gradient-to-br from-emerald-800 to-teal-950 p-10 rounded-[2.5rem] w-full max-w-lg shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/10 text-white relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h2 className="text-3xl font-black mb-8 uppercase tracking-tighter border-b border-white/10 pb-4">
                Operator Info
              </h2>

              <div className="space-y-6">
                <div className="bg-white/5 p-5 rounded-2xl border border-white/5">
                  <p className="text-[10px] uppercase tracking-[0.2em] font-black text-emerald-300 mb-1">
                    Full Name
                  </p>
                ...
                <div className="bg-white/5 p-5 rounded-2xl border border-white/5">
                  ...
                  <p className="font-bold">{selectedProvider.mandal_name || "—"}</p>
                </div>
              </div>
